...
class Scenario:
    ...
    def from_dict(cls, data: Dict) -> 'Scenario':
        ...
        sections = [Section(name=s['name'], seats=s['seats']) for s in data['sections']]
        return cls(
            name=data['name'],
...
def heuristic_price_search(target: float, sections: List[Section], 
    ...
    for _ in range(5_000):  # Reducido para mejor rendimiento
        ...
        for _ in range(len(sections) - 2):
            min_price = max(int(round(global_min)), int(prev_price / margin_factor * 0.95))
...
def generate_training_data(num_samples: int = 1000) -> List[Dict]:
    ...
    section_configs = [
        [{"name": "VIP", "seats": 100}, {"name": "Premium", "seats": 300}, {"name": "General", "seats": 600}],
        [{"name": "VIP", "seats": 150}, {"name": "Premium", "seats": 450}, {"name": "General", "seats": 900}],
        [{"name": "VIP", "seats": 200}, {"name": "Premium", "seats": 600}, {"name": "General", "seats": 1200}],
        [{"name": "VIP", "seats": 80}, {"name": "Premium", "seats": 240}, {"name": "General", "seats": 480}],
        [{"name": "VIP", "seats": 120}, {"name": "Premium", "seats": 360}, {"name": "General", "seats": 720}]
    ]
    
    for _ in range(num_samples):
        # Seleccionar una configuración aleatoria
        section_config = random.choice(section_configs)
        sections = [Section(name=s["name"], seats=s["seats"]) for s in section_config]
        ...
        combos = generate_valid_combinations(sections, "alta", global_min, global_max, margin_factor)
        ...
        if combos:
            ...
            revenue = calculate_revenue(tuple(best_combo), tuple(s.seats for s in sections), sell_rate)
...
def train_price_recommender(historical_data: List[Dict] = None, save_path: str = "price_recommender.joblib") -> RandomForestRegressor:
    ...
    for data in historical_data:
        features = [
            ...
            data['sell_rate'],
            data['target_income'],
            data['section_index']  # Posición de la sección (0 para VIP, 1 para Premium, etc.)
        ]
        X.append(features)
...
def import_scenario_from_json(json_str: str) -> Dict:
    ...
    try:
        ...
        st.error(f"Error al importar escenario: datos incompletos o incorrectos - {str(e)}")
        return None
...
def main():
    ...
    with st.sidebar:
        ...
        if st.session_state.current_scenario:
            json_str = export_scenario_to_json(st.session_state.current_scenario)
            st.download_button(
                "📤 Exportar Escenario Actual",
                json_str,
        ...
        if uploaded_file:
            ...
            if imported_scenario:
                ...
                st.session_state.section_names = [s.name for s in first_scenario.sections]
                st.session_state.section_seats = [s.seats for s in first_scenario.sections]
                st.success("Escenario importado correctamente y secciones sincronizadas")
                st.rerun()
    ...
    if len(st.session_state.section_names) != st.session_state.num_sections:
        if len(st.session_state.section_names) < st.session_state.num_sections:
            st.session_state.section_names += [f"Sección {i+1}" for i in range(len(st.session_state.section_names), st.session_state.num_sections)]
            st.session_state.section_seats += [500] * (st.session_state.num_sections - len(st.session_state.section_seats))
        else:
            st.session_state.section_names = st.session_state.section_names[:st.session_state.num_sections]
            st.session_state.section_seats = st.session_state.section_seats[:st.session_state.num_sections]
    ...
    for tab, (scenario_name, scenario_code) in zip(scenario_tabs, scenarios.items()):
        with tab:
            ...
            if combos:
                ...
                st.plotly_chart(
                    create_interactive_price_chart({scenario_name: scenario}, st.session_state.presentation_mode),
                ...
                if st.session_state.presentation_mode != 'minimal':
                    ...
                    for idx, combo in enumerate(top_combos, 1):
                        ...
                        if idx == 1:
                            st.markdown(f"**Opción {idx} - Mejor aproximación al ingreso objetivo**")
                        elif idx == 2:
                            st.markdown(f"**Opción {idx} - Segunda mejor aproximación**")
                        else:
                            st.markdown(f"**Opción {idx} - Tercera mejor aproximación**")
                    ...
                    st.info("Estas recomendaciones están basadas en un modelo de aprendizaje automático entrenado con datos sintéticos.")
                    ...
                    with col2:
                        ...
                        st.code(" | ".join(f"${int(p):,}" for p in recommended_prices))
                    ...
                    if abs(recommended_revenue - st.session_state.target) < abs(best_revenue - st.session_state.target):
                        st.success("¡Las recomendaciones de IA están más cerca del ingreso objetivo que la mejor opción encontrada!")
                    else:
                        st.info("La mejor opción encontrada está más cerca del ingreso objetivo que las recomendaciones de IA.")
        ...
        progress_bar.progress((list(scenarios.keys()).index(scenario_name) + 1) / len(scenarios))
    if len(all_scenarios) > 1:
        st.header("📊 Comparación de Escenarios")
        col1, col2 = st.columns(2)
        with col1:
            st.plotly_chart(
                create_interactive_price_chart(all_scenarios, st.session_state.presentation_mode),
                use_container_width=True
            )
        with col2:
            st.plotly_chart(
                create_revenue_comparison_chart(all_scenarios, st.session_state.presentation_mode),
                use_container_width=True
            )
    st.session_state.current_scenario = all_scenarios
    # Pedir nombre del show antes de descargar Excel
    if all_scenarios:
        show_name = st.text_input("Nombre del Show para el reporte Excel:", value="Show")
        if show_name:
            excel_file = generate_excel_report(all_scenarios, st.session_state.presentation_mode, show_name)
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        for scenario_name, scenario in scenarios.items():
            ...
            if presentation_mode != 'minimal':
                data['Tasa de Venta'] = [round(scenario.sell_rate, 2)] * len(scenario.sections)
                data['Asientos Vendidos'] = [int(s.seats * scenario.sell_rate) for s in scenario.sections]
                data['Ingreso (MXN)'] = [int(p * s.seats * scenario.sell_rate) for p, s in zip(scenario.prices, scenario.sections)]
                total_revenue = sum(data['Ingreso (MXN)'])
                if total_revenue > 0:
                    data['% Contribución'] = [round(r / total_revenue, 4) for r in data['Ingreso (MXN)']]
                else:
